...
let upcoming = events.filter(event => event.date > data.currentDate)
...
function renderCards(arrayData) {
    ...
    if(arrayData.length > 0){
        arrayData.forEach(card => {
            ...
            div.innerHTML = ` <a class="anchor" href="./details.html?id=${card._id}">
...
search.addEventListener('click', () => {
     let arrayFilterCheck = filterChecks(upcoming)
...
checks.forEach(checkCategory => {
    checkCategory.addEventListener('change', () => {
        let arrayFilterText = searchText (texto.value, upcoming)
